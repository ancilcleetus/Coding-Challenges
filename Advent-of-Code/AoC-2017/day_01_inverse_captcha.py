...
def get_inverse_captcha(digit_sequence: str) -> int:
    result = 0

    for current_digit, next_digit in zip(digit_sequence[:-1], digit_sequence[1:]):
        if current_digit == next_digit:
            result += int(current_digit)
    
    if digit_sequence[0] == digit_sequence[-1]:
        result += int(digit_sequence[0])

    return result


assert get_inverse_captcha("1122") == 3
assert get_inverse_captcha("1111") == 4
assert get_inverse_captcha("1234") == 0
assert get_inverse_captcha("91212129") == 9
...
if __name__ == "__main__":
    result =  get_inverse_captcha(PUZZLE_INPUT)
